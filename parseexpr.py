...
def recursive_generate(node, variable_dict={}, variable_number=0, sentences=[]):
    ...
    if isinstance(node, ast.Expr):
        ...
        if not isinstance(node.test, (ast.Num, ast.Name)):
            variable_number += 1
            recursive_generate(node.test, variable_dict, variable_number, sentences)
        if not isinstance(node.orelse, (ast.Num, ast.Name)):
            variable_number += 1
            recursive_generate(node.orelse, variable_dict, variable_number, sentences)
        if not isinstance(node.body, (ast.Num, ast.Name)):
            variable_number += 1
